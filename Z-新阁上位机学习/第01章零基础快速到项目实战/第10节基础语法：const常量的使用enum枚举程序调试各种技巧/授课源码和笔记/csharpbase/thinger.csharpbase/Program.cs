...
namespace thinger.csharpbase
{
    ...
    class Program
    {
        ...
        static void Test38()
        {
            ...
            string[] dataArray = data.Split(); //使用空格分割的时候不需要参数。
            string newData = string.Join("_", dataArray);
            Console.WriteLine(newData);
            Console.WriteLine("***************************************");
            data = "AB,EF,HU,00";
            dataArray = data.Split(',');//使用逗号分隔字符串
            newData = string.Join("&", dataArray);
            Console.WriteLine(newData);
            Console.WriteLine("数组元素个数：" + dataArray.Length);
            Console.WriteLine("***************************************");
            data = "AB,EF,HU,00";
            string dataInfo = data.Replace(',', '-');//字符串替换
            Console.WriteLine(dataInfo);

            //数组使用中常见问题
            // int[] netScore = new int[5] { 90, 56, 78 };//这行代码编译不通过。
            int[] netScore2 = new int[3] { 90, 56, 78 }; //正确的写法
            int myScore = netScore2[3];  
            
            //提示：System.IndexOutOfRangeException:“索引超出了数组界限。”
        ...
        static void Test43()
        {
            ...
            DayOfWeek week = DayOfWeek.Tuesday;
            switch (week)
            {
